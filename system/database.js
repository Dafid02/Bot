module.exports = (m) => {
   ...
   if (setting) {
      ...
      if (!('sk_pack' in setting)) setting.sk_pack = 'Yd-Sticker'
      if (!('sk_author' in setting)) setting.sk_author = '@Yd|xd'
      ...
      if (!('owners' in setting)) setting.owners = ['6281363609990']
      if (!isNumber(setting.lastReset)) setting.lastReset = new Date * 1
      if (!('msg' in setting)) setting.msg = ' *Halo👋* | +tag |\nSaya adalah (Bot whatsapp) Chat Otomatis'
      if (!isNumber(setting.menuStyle)) setting.menuStyle = 4
      if (!('cover' in setting)) setting.cover = 'https://yudibatam.files.wordpress.com/2023/05/images-3.jpeg'
      if (!('link' in setting)) setting.link = 'http://yudibatam.wordpress.com'
   } else {
      global.db.setting = {
         autodownload: true,
         chatbot: true,
         ...
         online: true,
         onlyprefix: '+',
         owners: ['6281363609990'],
         lastReset: new Date * 1,
         msg: ' *Halo👋* | +tag | \nSaya adalah (Bot Whatsapp) Chat Otomatis',
         menuStyle: 4,
         cover: 'https://yudibatam.files.wordpress.com/2023/05/images-3.jpeg',
         link: 'http://yudibatam.wordpress.com'
      }
   }
}